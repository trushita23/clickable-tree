...
export const DynamicTabs: React.FC<CheckBoxTreeViewConfig> = props => {
  const [items, loading] = hooks.useFetch(props.tabsUrl, [
    { label: "", value: "" }
  ]);
  if (loading) {
    return (
    ...
    return <SimpleTabs items={items} {...omit(props, "tabsUrl")} />;
  }
};
...
export const SimpleTabs: React.FC<TabProps> = props => {
  ...
  const [tabValue, setTabValue] = React.useState<string | number>(
    props.items[0].value
  );
  const tabs: Array<any> = [];
  forEach(props.items, (item: TabItem, key: number) =>
    tabs.push(
      <Tab
        key={`tab-${item.label}`}
        style={{ zIndex: 1000 }}
  ...
  const handleChangeDefault: TriggerFunction = (
    ...
    <React.Fragment>
      ...
      {tabValue ? (
        <TabPanel
          ...
          value={tabValue}
          {...omit(props, ["title", "items", "tabsUrl", "tabOnChange"])}
        />
      ) : (
        ""
      )}
